"""Multi-agent environment for ISAC with MIMO BS — 3-agent version."""
...
class ISACMultiAgentEnv:
    """Multi-agent environment for ISAC — 3 agents.
    ...
    def _setup_scenario(self):
        ...
        self.q_user = np.array([200.0, 0.0, 1.5])
        self.q_jammer = np.array([100.0, -120.0, 50.0])
        self.q_eves = np.array([
            [200.0, 150.0, 1.5],
            [150.0, -130.0, 1.5],
            [300.0, 80.0, 1.5],
        ])
    ...
    def _setup_spaces(self):
        ...
        self.agent_names = ["bs_beamformer", "uav_trajectory", "jammer_beamformer"]
        ar = c.action_range
        self.action_spaces = {
    ...
    def _compute_effective_channels_slot(self, q_uav_slot, seed_offset=0):
        c = self.cfg
        ...
        phi = design_ris_phases(h_BR, h_RU)
    ...
    def _build_obs_bs(self, q_uav, w_bs, v_jammer, sec, sense, cons_viol) -> np.ndarray:
        ...
        for n in range(min(1, c.N_time)):
            h_eff_user, _ = self._compute_effective_channels_slot(q_uav[n], n * 10)
        ...
        _, h_eff_eves = self._compute_effective_channels_slot(q_uav[0], 0)
        for ke in range(N_EVE):
    ...
    def _build_obs_uav_trajectory(self, q_uav, w_bs, v_jammer, sec, sense, cons_viol) -> np.ndarray:
        ...
        # Scene context: user position + eve mean positions
        parts.append(self.q_user.ravel().astype(np.float32))
        parts.append(np.mean(self.q_eves, axis=0).astype(np.float32))
