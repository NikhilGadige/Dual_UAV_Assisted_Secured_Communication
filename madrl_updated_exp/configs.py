"""Configuration dataclasses for MADRL experiments — updated 4-agent version.

Adds config knobs for the three previously-unwired pieces: Random-Walk
user mobility, HPPP eavesdropper density, and RIS phase-shift as a real
decision variable (see madrl_updated_exp/environment.py).
"""
...
class EnvConfig:
    ...
    user_speed_max: float = 3.0  # m/s cap for the Random-Walk mobile user
    eve_density_lambda: float = 2e-5  # HPPP intensity (eves per unit area), same default as fd_jammer_exp
    ...
    def __post_init__(self):
        if not self.agents:
            ...
            self.agents = [
                ...
                AgentConfig(name="ris_phase", act_dim=self.env.N_time * self.env.N_ris),
            ]
