...
The similarity between the original sentence's semantic features and the reconstructed features at the receiver is modeled by a sigmoidal function:
\begin{equation}
    S(\gamma) = \frac{1}{1 + \exp\left( -\left(\lambda_1 \gamma_{\text{dB}} + \lambda_2\right) \right)},
\end{equation}
where $\gamma_{\text{dB}} = 10\log_{10}(\gamma)$ and $\lambda_1, \lambda_2$ are scaling parameters capturing the linguistic complexity and compression capability of the deep semantic model. Intuitively, $S(\gamma) \in (0,1)$ saturates toward unity as the link SINR grows large, reflecting near-perfect meaning recovery under favorable channel conditions, and decays toward zero as $\gamma$ degrades, reflecting the collapse of semantic fidelity when the decoder can no longer reliably infer the transmitted concepts from the received symbols. The steepness of this transition is controlled by $\lambda_1$, while $\lambda_2$ shifts the operating point at which the decoder transitions from unreliable to reliable semantic recovery.
