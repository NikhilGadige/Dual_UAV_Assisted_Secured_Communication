...
\begin{equation}
    P_d^{(e)} = w_3 P_{d,E}^{(e)} + w_4 P_{d,T}^{(e)}, \qquad w_3 + w_4 = 1,
\end{equation}
...
\begin{equation}
    U^{(e)} \triangleq \lambda_1 \,\text{ASSR}^{(e)} + \lambda_2 \,P_d^{(e)}, \qquad \lambda_1 + \lambda_2 = 1.
\end{equation}
...
\begin{subequations}
\begin{align}
    \max_{\mathbf{q}_R, \mathbf{q}_J, P_{BS}, P_J, a_F} \quad & \frac{1}{E} \sum_{e=1}^{E} U^{(e)} = \frac{1}{E} \sum_{e=1}^{E} \left[ \lambda_1 \,\text{ASSR}^{(e)} + \lambda_2 \,P_d^{(e)} \right] \\
...
\end{align}
\end{subequations}
where $CRB_{\max} = 1.0 \times 10^8$ and $d_{min} = 10.0$\,m, matching the original constraint values. Section~\ref{sec:convergence} reports results with $\lambda_1 = \lambda_2 = 0.5$ and $w_3 = w_4 = 0.5$.
...
\begin{itemize}
    ...
    \item Single-Agent SAC converges only slightly slower (around episode 640), reaching a comparable steady-state rolling-average utility of about $0.14$, the highest sustained value among all algorithms, consistent with its raw secrecy rate also peaking highest (up to $0.61$ M-suts/s) among the compared methods.
    \item MAPPO takes considerably longer to ramp up (around episode 1350) and remains the most volatile of the four, swinging between roughly $0.07$ and its peak of $0.20$ utility, and ending training around $0.11$ -- higher peak potential, but less stable than MATD3PG or SAC over a long run.
    \item Single-Agent TD3PG converges early (around episode 300) but plateaus at the lowest steady-state utility of the four, around $0.06$, reflecting a comparatively weaker balance of secrecy and detection performance once its policy stabilizes.
\end{itemize}
These results, computed over a full $8000$-episode training run per algorithm, revise the earlier raw-secrecy-only reading of this experiment: SAC and MATD3PG -- not MAPPO -- sustain the strongest joint secrecy/sensing utility once both objective terms are accounted for, and MAPPO's high raw secrecy rate does not translate into a proportionally higher or more stable combined utility.
