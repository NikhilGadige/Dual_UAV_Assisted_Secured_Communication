...
\begin{figure}[H]
    ...
    \includegraphics[width=0.6\textwidth]{system-model.jpg}
...
The cascaded BS-to-RIS-to-User link utilizes passive analog reflecting beamforming at the RIS. Let the RIS phase shift matrix (beamforming weight matrix) be denoted as $\mathbf{\Phi}(t) = \text{diag}\left( \beta_1 e^{j \phi_1(t)}, \beta_2 e^{j \phi_2(t)}, \dots, \beta_N e^{j \phi_N(t)} \right)$, where $\beta_n \in [0, 1]$ is the amplitude reflection coefficient (assumed to be 1 for ideal reflection) and $\phi_n(t) \in [0, 2\pi)$ is the phase shift of the $n$-th reflecting element. The cascaded channel gain is:
\begin{equation}
    g_i(t) = | \mathbf{h}_{BR}^H(t) \mathbf{\Phi}(t) \mathbf{h}_{Ri}(t) |^2, \quad i \in \{T, U\},
\end{equation}
where $\mathbf{h}_{BR}(t) \in \mathbb{C}^{N \times 1}$ and $\mathbf{h}_{Ri}(t) \in \mathbb{C}^{N \times 1}$ are the multi-antenna channel vectors from BS-to-RIS and RIS-to-user $i$ respectively. Under optimal phase alignment (coherent beamforming), the phase shifts are configured as $\phi_n(t) = - (\angle h_{BR, n}(t) + \angle h_{Ri, n}(t))$, aligning the phases of all reflected paths. This results in the maximized coherent channel gain:
\begin{equation}
    g_i(t) = h_{BR}(t) \cdot h_{Ri}(t) \cdot N^2, \quad i \in \{T, U\},
\end{equation}
where $h_{BR}(t)$ and $h_{Ri}(t)$ are the path loss gains, and the factor $N^2$ represents the passive array beamforming reflecting gain. The received jamming interference power from Jammer $J$ is $I_{jam, i}(t) = P_J(t) h_{Ji}(t)$.
...
Ground eavesdroppers $E_k$ ($k=1,2,3$) attempt to decode both $s_F$ and $s_N$. In practice, the exact locations of the eavesdroppers are unknown to the legitimate nodes, and only their estimated coordinates $\tilde{\mathbf{w}}_{E_k} = [\tilde{x}_{E_k}, \tilde{y}_{E_k}]^T$ can be obtained. The true position $\mathbf{w}_{E_k}$ of eavesdropper $E_k$ is assumed to lie within a disk of uncertainty radius $\Delta Q_k$ centered at $\tilde{\mathbf{w}}_{E_k}$:
\begin{equation}
    \|\mathbf{w}_{E_k} - \tilde{\mathbf{w}}_{E_k}\| \le \Delta Q_k.
\end{equation}
To evaluate security in the worst-case scenario, the channel gains for the eavesdropper interception links are optimized (maximized) over the uncertainty region, while the jamming channel gain is minimized to limit the artificial noise power received.
Specifically, for the transmitter-to-eavesdropper links (e.g., RIS-to-Eve), the worst-case 2D horizontal distance is minimized:
\begin{equation}
    d_{R, E_k}^{2D, \min}(t) = \max\left( \|\mathbf{q}_{R, 2D}(t) - \tilde{\mathbf{w}}_{E_k}\| - \Delta Q_k, 0 \right).
\end{equation}
The cascaded channel gain $g_{E_k}(t)$ is computed using the modified 3D distance $d_{R, E_k}^{\min}(t) = \sqrt{(d_{R, E_k}^{2D, \min}(t))^2 + z_R^2(t)}$:
\begin{equation}
    g_{E_k}(t) = h_{BR}(t) \cdot h_{RE_k}^{\max}(t) \cdot N^2,
\end{equation}
where $h_{RE_k}^{\max}(t)$ incorporates the minimized path loss using $d_{R, E_k}^{\min}(t)$.
Conversely, for the jammer-to-eavesdropper link, the worst-case 2D distance is maximized to minimize the jamming interference power received by the eavesdropper:
\begin{equation}
    d_{J, E_k}^{2D, \max}(t) = \|\mathbf{q}_{J, 2D}(t) - \tilde{\mathbf{w}}_{E_k}\| + \Delta Q_k.
\end{equation}
The jamming power at $E_k$ is computed using the modified 3D distance $d_{J, E_k}^{\max}(t) = \sqrt{(d_{J, E_k}^{2D, \max}(t))^2 + z_J^2(t)}$:
\begin{equation}
    I_{jam, E_k}(t) = P_J(t) h_{JE_k}^{\min}(t),
\end{equation}
where $h_{JE_k}^{\min}(t)$ incorporates the maximized path loss using $d_{J, E_k}^{\max}(t)$.

Eavesdropper $E_k$ decodes Far User's signal $s_F$ with:
\begin{equation}
...
Our objective is to jointly optimize the trajectories $\{\mathbf{q}_R(t), \mathbf{q}_J(t)\}$ and the power allocation parameters $\{P_{BS}(t), P_J(t), a_F(t)\}$ to maximize both the achieved secrecy rate and the target/eavesdropper detection probability of the network, subject to sensing quality, collision avoidance, and transmission power constraints. For a training run consisting of $E$ episodes, let
\begin{equation}
    R_{sec}^{(e)} = \frac{1}{T_{total}} \sum_{t=1}^{T_{total}} R_{sec}(t)
\end{equation}
denote the average secrecy rate achieved during episode $e \in \{1, \dots, E\}$. Since the secrecy rate and the detection probability operate on different numerical ranges, the secrecy rate is normalized by the maximum value achieved anywhere across the entire run:
\begin{equation}
    R_{\max} = \max_{j \in \{1, \dots, E\}} R_{sec}^{(j)}, \qquad
    \text{ASSR}^{(e)} = \frac{R_{sec}^{(e)}}{R_{\max}} \in [0, 1],
\end{equation}
where $\text{ASSR}^{(e)}$ (Average Sum Secrecy Ratio) is the normalized secrecy rate ratio for episode $e$. The combined detection probability for episode $e$ is a weighted sum of the average eavesdropper detection probability $P_{d,E}^{(e)}$ and the Target detection probability $P_{d,T}^{(e)}$:
\begin{equation}
    P_d^{(e)} = \lambda_1 P_{d,E}^{(e)} + \lambda_2 P_{d,T}^{(e)}, \qquad \lambda_1 + \lambda_2 = 1,
\end{equation}
with $P_{d,E}^{(e)} = \frac{1}{3}\sum_{k=1}^3 P_{d,E_k}^{(e)}$. Both $\text{ASSR}^{(e)}$ and $P_d^{(e)}$ lie in $[0,1]$, so the per-episode multi-objective utility, combining the normalized secrecy performance with the detection probability, is denoted:
\begin{equation}
    U^{(e)} \triangleq w_1 \,\text{ASSR}^{(e)} + w_2 \,P_d^{(e)}, \qquad w_1 + w_2 = 1.
\end{equation}
The joint optimization problem is therefore to maximize this combined secrecy-and-detection utility, averaged over the run, subject to the Cram\'{e}r-Rao Bound constraint on target localization accuracy, the minimum inter-UAV separation, and the transmit power / NOMA allocation bounds -- none of which change from the original formulation:
\begin{subequations}
\begin{align}
    \max_{\mathbf{q}_R, \mathbf{q}_J, P_{BS}, P_J, a_F} \quad & \frac{1}{E} \sum_{e=1}^{E} U^{(e)} = \frac{1}{E} \sum_{e=1}^{E} \left[ w_1 \,\text{ASSR}^{(e)} + w_2 \,P_d^{(e)} \right] \\
...
\end{align}
\end{subequations}
where $CRB_{\max} = 1.0 \times 10^8$ and $d_{min} = 10.0$\,m, matching the original constraint values. Section~\ref{sec:convergence} reports results with $w_1 = w_2 = 0.5$ and $\lambda_1 = \lambda_2 = 0.5$.
...
\begin{enumerate}
    ...
    \item \textbf{SATD3PG}: A continuous, deterministic actor-critic algorithm that uses twin critics to bound overestimation bias and delayed policy updates to stabilize learning.
    \item \textbf{SASAC}: An off-policy maximum entropy actor-critic algorithm that optimizes for both expected return and policy entropy $\mathcal{H}(\pi(\cdot|s))$, ensuring robust exploration and escaping local minima.
...
AWGN Noise Power ($\sigma^2$) & $10^{-10}$ W ($-70$ dBm) \\
...
The five algorithms were evaluated over training episodes to compare their capability to escape the zero-secrecy landscape. Per the Problem Formulation of Section~\ref{sec:convergence}, the metric that reflects the actual joint optimization objective is the multi-objective utility $U^{(e)} = \lambda_1 \,\text{ASSR}^{(e)} + \lambda_2 \,P_d^{(e)}$, not the raw secrecy rate alone: raw secrecy and the detection probability live on different scales, and plotting secrecy in isolation silently drops the sensing/detection term of the objective. The 100-episode rolling average of $U^{(e)}$ (with $\lambda_1 = \lambda_2 = 0.5$, $w_3 = w_4 = 0.5$) is plotted in Fig.~\ref{fig:rolling_secrecy} for the four RL algorithms (MAPPO, MATD3PG, SASAC, and SATD3PG); Random Walk is excluded as its utility is negligible throughout.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{output_final/rolling_secrecy_comparison.png}
    \caption{Convergence comparison showing the rolling 100-episode average multi-objective utility $U = \lambda_1 \,\text{ASSR} + \lambda_2 \,P_d$ for all four RL algorithms (MAPPO, MATD3PG, SASAC, and SATD3PG) on the Week 9 system model.}
    \label{fig:rolling_secrecy}
\end{figure}

The convergence curves demonstrate that:
\begin{itemize}
    \item Random Walk fails to establish secure communication, keeping raw secrecy rate close to zero (below $10^{-6}$ M-suts/s throughout the run), so its utility never rises appreciably above zero.
    \item MATD3PG converges fastest, reaching roughly half its eventual peak utility by around episode 140 and settling into a stable rolling-average utility of about $0.13$ for the remainder of training.
    \item SASAC converges only slightly slower (around episode 640), reaching a comparable steady-state rolling-average utility of about $0.14$, the highest sustained value among all algorithms, consistent with its raw secrecy rate also peaking highest (up to $0.61$ M-suts/s) among the compared methods.
    \item MAPPO takes considerably longer to ramp up (around episode 1350) and remains the most volatile of the four, swinging between roughly $0.07$ and its peak of $0.20$ utility, and ending training around $0.11$ -- higher peak potential, but less stable than MATD3PG or SASAC over a long run.
    \item SATD3PG converges early (around episode 300) but plateaus at the lowest steady-state utility of the four, around $0.06$, reflecting a comparatively weaker balance of secrecy and detection performance once its policy stabilizes.
\end{itemize}
These results, computed over a full $8000$-episode training run per algorithm, revise the earlier raw-secrecy-only reading of this experiment: SASAC and MATD3PG -- not MAPPO -- sustain the strongest joint secrecy/sensing utility once both objective terms are accounted for, and MAPPO's high raw secrecy rate does not translate into a proportionally higher or more stable combined utility.

\section{Conclusion}
In this paper, we investigated joint trajectory and power optimization for secure multi-agent UAV networks under an integrated sensing and communication (ISAC) architecture. By normalizing the observation space and incorporating direct SNR-JNR guidance, we successfully resolved training saturation and escaped deceptive reward local minima. We provided a head-to-head comparison of five reinforcement learning paradigms. The simulation results demonstrated that continuous-action maximum-entropy algorithms (such as SASAC) are highly suited for secure ISAC networks, achieving fast convergence and establishing high system secrecy rates.
