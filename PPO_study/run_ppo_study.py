...
from PPO_study.plot_ppo import generate_channel_comparison, generate_single_run_plots
...
def run_one_channel(channel: str, episodes: int, hidden_dim: int, seed: int, device: str, out_root: Path) -> dict:
    run_dir = out_root / f"ppo_{channel}_h{hidden_dim}"
    cfg = PPOConfig(
        episodes=episodes,
    ...
    summary = train_ppo(cfg, output_dir=str(run_dir))
    title = f"PPO + {channel.title()} (h{hidden_dim})"
    color = "#1f77b4" if channel == "rician" else "#d62728"
    generate_single_run_plots(summary["training_log_csv"], str(run_dir), title, color)
    return summary


def run_study(episodes: int, hidden_dim: int, seed: int, device: str, out_root: str = "ppo_study/output") -> dict:
    root = Path(out_root)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run PPO convergence study for Rayleigh and Rician channels")
    parser.add_argument("--episodes", type=int, default=2500)
    parser.add_argument("--hidden-dim", type=int, default=64)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--device", type=str, default="cpu")
    parser.add_argument("--output-dir", type=str, default="ppo_study/output")
