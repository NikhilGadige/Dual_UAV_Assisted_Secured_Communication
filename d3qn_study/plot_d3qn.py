...
def generate_channel_comparison(rayleigh_csv: str, rician_csv: str, output_dir: str) -> str:
    ...
    _style()
    fig, ax = plt.subplots(figsize=(8.5, 5.0))
    ax.plot([r["episode"] for r in ray_rows], [r["rolling100"] for r in ray_rows], label="D3QN + Rayleigh", color="#d62728")
    ax.plot([r["episode"] for r in ric_rows], [r["rolling100"] for r in ric_rows], label="D3QN + Rician", color="#1f77b4")
    ax.set_title("D3QN Convergence: Rayleigh vs Rician")
    ax.set_xlabel("Episode")
    ...
    p = out / "d3qn_channel_comparison_rolling100.png"
    fig.savefig(p, bbox_inches="tight")
    plt.close(fig)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate D3QN convergence plots")
    parser.add_argument("--csv", type=str, required=True)
    parser.add_argument("--output-dir", type=str, required=True)
    parser.add_argument("--title", type=str, default="D3QN")
