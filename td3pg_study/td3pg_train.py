...
def exploration_noise(global_step: int, cfg: TD3PGStudyConfig) -> float:
    if global_step >= cfg.exploration_noise_decay_steps:
        return cfg.exploration_noise_end
    frac = global_step / max(cfg.exploration_noise_decay_steps, 1)
    return cfg.exploration_noise_start + frac * (cfg.exploration_noise_end - cfg.exploration_noise_start)


def evaluate_actor(actor: Actor, cfg: TD3PGStudyConfig, device: torch.device, episodes: int, seed_offset: int) -> dict:
    actor.eval()
    avg_secrecy = []
    payload = []
    with torch.no_grad():
...
def train_td3pg(cfg: TD3PGStudyConfig | None = None, output_dir: str | None = None) -> dict:
    cfg = cfg or TD3PGStudyConfig()
    set_seed(cfg.seed)
    ...
    actor_target = Actor(state_dim, action_dim, cfg.hidden_dim).to(device)
    ...
    for episode in range(1, cfg.episodes + 1):
        ...
        while not done:
            ...
            with torch.no_grad():
                state_t = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
                action = actor(state_t).cpu().numpy()[0]
            noise_std = exploration_noise(global_step, cfg)
            ...
            if len(replay) >= cfg.min_replay_size:
                ...
                with torch.no_grad():
                    target_noise = torch.clamp(
                        torch.randn_like(actions_t) * cfg.target_policy_noise,
                        -cfg.target_noise_clip,
                        cfg.target_noise_clip,
                    )
                    next_actions = torch.clamp(actor_target(next_states_t) + target_noise, -1.0, 1.0)
                    target_q = torch.min(
                        critic1_target(next_states_t, next_actions),
                        critic2_target(next_states_t, next_actions),
                    )
                    q_target = rewards_t + (1.0 - dones_t) * cfg.gamma * target_q
                ...
                if global_step % cfg.policy_delay == 0:
                    actor_loss = -critic1(states_t, actor(states_t)).mean()
                    actor_opt.zero_grad()
                    actor_loss.backward()
        ...
        if episode == 1 or episode % 25 == 0 or episode == cfg.episodes:
            print(
                ...
                f"noise={noise_std:.3f} | avg_R_sec={avg_rsec:.3f} Mbps | "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TD3PG convergence study for dual-UAV secure uplink.")
...
if __name__ == "__main__":
    ...
    cfg = TD3PGStudyConfig(
        ...
        exploration_noise_decay_steps=args.episodes * 200,
    )
    train_td3pg(cfg)
