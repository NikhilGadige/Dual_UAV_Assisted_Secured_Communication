        ...
        parsed.append(
            {
                ...
                "entropy": _float_or_none(row.get("policy_entropy")),
            }
        )
    return parsed
...
def generate_single_run_plots(csv_path: str, output_dir: str) -> list[str]:
    ...
    saved = [
        ...
        _save_line(episodes, alpha, label, "Entropy Temperature Alpha", out_dir / "alpha_vs_episode.png", color),
    ]
...
def generate_channel_comparison(output_root: str = "sac_study/output") -> list[str]:
    root = Path(output_root)
    csvs = sorted(root.glob("sac_*_h*/sac_training_log.csv"))
    if len(csvs) < 2:
        return []
    ...
    ax.set_title("SAC Training Convergence: Rician vs Rayleigh")
    ax.set_xlabel("Episode")
    ax.set_ylabel("Rolling 100-Episode Secrecy Rate (Mbps)")
    ax.legend(frameon=True)
    ax.margins(x=0.01)
    fig.tight_layout()
    path = plots_dir / "sac_rician_vs_rayleigh_rolling100.png"
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
    saved.append(str(path.resolve()))
    ...
    ax.set_title("SAC Evaluation Convergence: Rician vs Rayleigh")
    ax.set_xlabel("Episode")
    ax.set_ylabel("Evaluation Secrecy Rate (Mbps)")
    ax.legend(frameon=True)
    ax.margins(x=0.01)
    fig.tight_layout()
    path = plots_dir / "sac_rician_vs_rayleigh_evaluation.png"
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
    saved.append(str(path.resolve()))
    return saved
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate SAC convergence plots.")
